    ...
    if Delegacion.objects.filter(pk__exact = delegation_id).exists():
        ...
        if Cargo.objects.filter(entidadCargo__entidad__iexact = 'delegación').exists():
            ...
            for cargo_obj in cargo_objs:
                ...
                if PersonalExterno.objects.filter(cargo__nombreCargo__exact = cargo_name, grupoAsociado__diocesisDependiente__delegacionDependiente__pk__exact = delegation_id).exclude(personalActivo=False).exists():
                    user_name = PersonalExterno.objects.filter(cargo__nombreCargo__exact = cargo_name, grupoAsociado__diocesisDependiente__delegacionDependiente__pk__exact = delegation_id).exclude(personalActivo=False).last().get_personal_name()
                else:
...
def get_id_actividad_name():
    ...
    if Actividad.objects.all().exists():
        ...
        for actividad_obj in actividad_objs:
            actividad_data.append([actividad_obj.get_actividad_id(), actividad_obj.get_actividad_name()])
