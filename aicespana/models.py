...
class Cargo(models.Model):
    entidadCargo = models.ForeignKey(EntidadesConCargo, on_delete=models.CASCADE)
...
class DelegacionManager(models.Manager):
    def create_new_delegacion(self, data):
        new_delegacion = self.create(
            nombreDelegacion=data["nombre"], imagenDelegacion=data["imagen"]
        )
...
class DiocesisManager(models.Manager):
    def create_new_diocesis(self, data):
        if Delegacion.objects.filter(pk__exact=data["delegation_id"]).exists():
            delegation_obj = Delegacion.objects.filter(
                pk__exact=data["delegation_id"]
            ).last()
        new_diocesis = self.create(
            nombreDiocesis=data["name"], delegacionDependiente=delegation_obj
        )
        return new_diocesis


class Diocesis(models.Model):
    delegacionDependiente = models.ForeignKey(Delegacion, on_delete=models.CASCADE)
    ...
    def get_diocesis_depending_groups(self):
        ...
        if Grupo.objects.filter(diocesisDependiente=self, grupoActivo=True).exists():
            groups = Grupo.objects.filter(
                diocesisDependiente=self, grupoActivo=True
            ).values("nombreGrupo")
        return groups
...
class ParroquiaManager(models.Manager):
    def create_new_parroquia(self, data):
        new_parroquia = self.create(
            diocesisDependiente=data["diocesis_obj"],
            nombreParroquia=data["nombre"],
            calle=data["calle"],
...
class Parroquia(models.Model):
    diocesisDependiente = models.ForeignKey(Diocesis, on_delete=models.CASCADE)
    ...
    def get_parroquia_full_data(self):
        return [
            ...
            self.get_parroquia_id(),
            self.get_diocesis_name(),
            self.get_diocesis_id(),
            self.calle,
            self.poblacion,
            self.provincia,
            self.codigoPostal,
    ...
    def update_parroquia_data(self, data):
        self.diocesisDependiente = Diocesis.objects.filter(
            pk__exact=data["diocesisID"]
        ).last()
...
class Grupo(models.Model):
    ...
    diocesisDependiente = models.ForeignKey(
        Diocesis, on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    def __str__(self):
        return (
            "%s" % (self.nombreGrupo)
            + " en diocesis"
            + self.diocesisDependiente.nombreDiocesis
        )
...
class Proyecto(models.Model):
    ...
    fotografiaProyecto = models.FileField(
        storage=memory_project_path_location, null=True, blank=True
    )
    fechaAlta = models.DateField(auto_now=False, null=True, blank=True)
    fechaBaja = models.DateField(auto_now=False, null=True, blank=True)
    ...
    def get_proyecto_full_data(self):
        ...
        if self.memoriaProyecto:
            memoria_proyecto = os.path.join(
                settings.MEDIA_URL, str(self.memoriaProyecto)
            )
        else:
            memoria_proyecto = ""
        if self.fotografiaProyecto:
            fotografia_proyecto = os.path.join(
                settings.MEDIA_URL, str(self.fotografiaProyecto)
            )
        else:
            fotografia_proyecto = ""
        data = [
...
class Actividad(models.Model):
    ...
    fotografiaActividad = models.FileField(
        storage=memory_project_path_location, null=True, blank=True
    )
    fechaAlta = models.DateField(auto_now=False, null=True, blank=True)
    fechaBaja = models.DateField(auto_now=False, null=True, blank=True)
    ...
    def get_actividad_full_data(self):
        ...
        if self.memoriaActividad:
            memoria_actividad = os.path.join(
                settings.MEDIA_URL, str(self.memoriaActividad)
            )
        else:
            memoria_actividad = ""
        if self.fotografiaActividad:
            fotografia_actividad = os.path.join(
                settings.MEDIA_URL, str(self.fotografiaActividad)
            )
        else:
...
class PersonalExternoManager(models.Manager):
    def create_new_external_personel(self, data):
        if TipoColaboracion.objects.filter(pk__exact=data["tipoColaboracion"]).exists():
            tipoColaboracion_obj = TipoColaboracion.objects.get(
                pk__exact=data["tipoColaboracion"]
            )
        else:
        ...
        new_ext_personel = self.create(
            ...
            fechaAlta=data["alta"],
            email=data["email"],
            telefonoFijo=data["fijo"],
            telefonoMovil=data["movil"],
            tipoColaboracion=tipoColaboracion_obj,
...
class PersonalExterno(models.Model):
    ...
    def update_all_data_for_voluntary(self, data):
        ...
        if TipoColaboracion.objects.filter(pk__exact=data["colaboracion_id"]).exists():
            tipoColaboracion_obj = TipoColaboracion.objects.get(
                pk__exact=data["colaboracion_id"]
            )
        else:
            tipoColaboracion_obj = None
        if data["grupoID"] != "":
        ...
        if data["proyectoID"] == "":
            ...
            if Proyecto.objects.filter(pk__exact=data["proyectoID"]).exists():
                proyecto_obj = Proyecto.objects.filter(
                    pk__exact=data["proyectoID"]
                ).last()
            else:
                proyecto_obj = None
        if data["actividadID"] == "":
            ...
            if Actividad.objects.filter(pk__exact=data["actividadID"]).exists():
                actividad_obj = Actividad.objects.filter(
                    pk__exact=data["actividadID"]
                ).last()
            else:
                actividad_obj = None
...
class PersonalIglesia(models.Model):
    ...
    delegacion = models.ForeignKey(
        Delegacion, on_delete=models.CASCADE, null=True, blank=True
    )
